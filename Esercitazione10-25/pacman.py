...
class Ghost(Actor):
    ...
    def move(self):
        if not pacman.position() == (112, 184) and (pacman._dx == 0 or pacman._dy == 0):
...
def spawn_ghosts():
    ...
    arena.add(Ghost(arena, (96,88), 'pink'))
    arena.add(Ghost(arena, (120,88), 'cyan'))
    arena.add(Ghost(arena, (144,88), 'orange'))
...
def draw_lives():
    g2d.set_color((0, 0, 0))
    g2d.fill_rect((0, 256), (232, 16))
    w = 232 - 16
    h = 256
    for i in range(pacman.lives()):
        g2d.draw_image_clip("https://tomamic.github.io/images/sprites/pac-man.png", (128, 16), (16, 16), (w, h))
        w -= 16

def tick():
    if not is_game_over(False):
